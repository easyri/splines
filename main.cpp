...
float spline_j(float x, float xj, float xj1){
    if( xj <= x && x <= xj1){
        return (x - xj1) / (xj - xj1);
    }
...
float spline_j1(float x, float xj, float xj1){
    if( xj <= x && x <= xj1){
        return (x - xj) / (xj1 - xj);
    }
    cout << "segmentation error";
...
float u(float x, float xj, float xj1, float uj, float uj1){
    return uj * spline_j(x, xj, xj1) + uj1 * spline_j1(x, xj, xj1);
}
...
uint8_t* enlarge_2x(uint8_t* cutdata, int width, int height, int n){
    ...
    for(int channel = 0; channel < n; ++channel){
        ...
        while(i < width * height * 4 * n){
            int j = n;
            while(j < width * 2 * n - 3){
                data[i + j + channel] = (uint8_t)u(j, j-1, j+1,
                                          data[i + j - n + channel],
                                          data[i + j + n + channel]);
                j += n * 2;
            }
    ...
    for(int channel = 0; channel < n; ++channel){
        ...
        while(i < actual_width - n){
            int j = 1;
            while(j < height * 2 - 1){
                data[i + actual_width * j + channel] = (uint8_t)u(j, j - 1, j + 1,
...
uint8_t* enlarge2x_third_order(uint8_t* cutdata, int width, int height, int n){
    ...
    for(int channel = 0; channel < n; ++channel){
        ...
        while(i < width * height * 4 * n){
            int j = n;
            while(j < width * n){
                data[i + j + channel]  = (uint8_t)left_u(j, j - 1, j + 1, j + 3, // 3?2
                                               data[i + j - n + channel],
                                               data[i + j + n + channel],
            ...
            while(j < width * 2 * n - 3){
                data[i + j + channel]  = (uint8_t)right_u(j, j - 1, j + 1, j - 3,
                                                         data[i + j - n + channel],
                                                         data[i + j + n + channel],
                                                         data[i + j - 3 * n + channel]);
                j += n * 2;
    ...
    for(int channel = 0; channel < n; ++channel){
        ...
        while(i < actual_width - n){
            int j = 1;
            while(j < height){
                data[i + actual_width * j + channel] = (uint8_t)left_u(j, j - 1, j + 1, j + 3,
                                                                       data[i + actual_width * (j - 1) + channel],
                                                                       data[i + actual_width * (j + 1) + channel],
            ...
            while(j < height * 2 - 1){
                data[i + actual_width * j + channel] = (uint8_t)right_u(j, j - 1, j + 1, j - 3,
                                                                        data[i + actual_width * (j - 1) + channel],
                                                                        data[i + actual_width * (j + 1) + channel],
                                                                        data[i + actual_width * (j - 3) + channel]);
                j += 2;
...
#define CHANNEL_NUM 3
    ...
    string filename2 = "C:\\Users\\belyn\\uimages\\monkey2.jpg";

    // ... x = width, y = height, n = # 8-bit components per pixel
    int x,y,n;
    Timer t1;
    unsigned char *data = stbi_load(filename2.c_str(), &x, &y, &n, 0);
    cout<<x<<" "<<y<<" "<<n<<endl;
    uint8_t* dataint = (uint8_t*)data;
    cout<< +dataint[0]<<" "<< +dataint[1]<<" "<< +dataint[2]<<endl;
...
//    stbi_write_jpg("enlargedimg1.jpg", x, y, 3, enlarged_data, 100);
    ...
    stbi_write_jpg("enlargedmonkey2.jpg", width * 2, height * 2, 3, enlarged_data2, 100);
    cout<<"time taken for the whole process: " << t1.elapsed()<<endl; // 2.4, 1.9, 2.1, 1.9
